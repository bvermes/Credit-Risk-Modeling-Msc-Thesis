...
def split_vec_todf(vec):
    ...
    if vec is not None:
        ...
        b = (
            pd.DataFrame([i.split("%,%") for i in vec], index=vec)
            .stack()
...
def dtm_binning_sv(dtm, breaks, spl_val):
    ...
    if spl_val is not None:
        ...
        dtm = (
            dtm[~dtm.index.isin(dtm_sv.index)].reset_index()
            if len(dtm_sv.index) < len(dtm.index)
            else None
        )
        ...
        binning_sv = (
            pd.merge(
                ...
                .groupby(["variable", "value"], observed=False, group_keys=False)["y"]
                .agg([n0, n1])
                .reset_index()
            ...
            .groupby(["variable", "rowid", "bin_chr"], observed=False, group_keys=False)
...
def check_empty_bins(dtm, binning):
    ...
    if binleft != binright:
        ...
        binning = (
            dtm.groupby(["variable", "bin"], observed=False, group_keys=False)["y"]
            .agg([n0, n1])
            .reset_index()
            .rename(columns={"n0": "good", "n1": "bad"})
        )
...
def woebin2_breaks(dtm, breaks, spl_val):
    ...
    if is_numeric_dtype(dtm["value"]):
        ...
        bstbrks = (
            ["-inf"]
            + list(
                set(bk_df.value.tolist()).difference(
                    set([np.nan, "-inf", "inf", "Inf", "-Inf"])
                )
            )
            + ["inf"]
        )
        ...
        if bk_df["value"].isnull().any():
            binning = (
                binning.assign(
                    bin=lambda x: [i if i != "nan" else "missing" for i in x["bin"]]
                )
                .fillna("missing")
        ...
        binning = (
            pd.merge(dtm, bk_df.assign(bin=lambda x: x.bin_chr), how="left", on="value")
            .fillna("missing")
            .groupby(["variable", "rowid", "bin"], observed=False, group_keys=False)[
                "y"
            ]
            .agg([n0, n1])
            .rename(columns={"n0": "good", "n1": "bad"})
...
def woebin2_init_bin(dtm, init_count_distr, breaks, spl_val):
    ...
    binning_sv = dtm_binsv_list["binning_sv"]
    if dtm is None:
        return {"binning_sv": binning_sv, "initial_binning": None}
    # binning
    if is_numeric_dtype(dtm["value"]):  # numeric variable
        ...
        xvalue_rm_outlier = xvalue[
            (xvalue >= iq[prob_down] - 3 * iqr) & (xvalue <= iq[prob_up] + 3 * iqr)
        ]
        ...
        # initial breaks
        brk = (
            np.unique(xvalue_rm_outlier)
            if len_uniq_x < 10
            else pretty(min(xvalue_rm_outlier), max(xvalue_rm_outlier), n)
        )

        brk = list(
            filter(lambda x: x > np.nanmin(xvalue) and x <= np.nanmax(xvalue), brk)
        )
        ...
        dtm.loc[:, "bin"] = pd.cut(
            dtm["value"], brk, right=False, labels=labels
        )  # .astype(str)
        # init_bin
        init_bin = (
            dtm.groupby("bin", observed=False, group_keys=False)["y"]
            .agg([n0, n1])
            .reset_index()
            .rename(columns={"n0": "good", "n1": "bad"})
        )
        ...
        init_bin = init_bin.assign(
            ...
            brkp=lambda x: [
                float(re.match("^\[(.*),.+", i).group(1)) for i in x["bin"]
            ],
        ...
        # initial binning datatable
        init_bin = (
            dtm.groupby("value", observed=False, group_keys=False)["y"]
            .agg([n0, n1])
            .rename(columns={"n0": "good", "n1": "bad"})
            .assign(
                variable=dtm["variable"].values[0],
    ...
    while len(init_bin.query("(good==0) or (bad==0)")) > 0:
        ...
        rm_brkp = (
            init_bin.assign(count=lambda x: x["good"] + x["bad"])
            .assign(
        ...
        init_bin = init_bin.assign(
            brkp2=lambda x: x["brkp"].shift(shift_period)
        ).assign(
            brkp=lambda x: np.where(x["brkp"] == rm_brkp["brkp"], x["brkp2"], x["brkp"])
        )
        # groupby brkp
        init_bin = (
            init_bin.groupby("brkp", observed=False, group_keys=False)
            .agg(
                {
...
def woebin2_tree_add_1brkp(dtm, initial_binning, count_distr_limit, bestbreaks=None):
    ...
    def total_iv_all_breaks(initial_binning, bestbreaks, dtm_rows):
        # best breaks set
        breaks_set = set(initial_binning.brkp).difference(
            set(list(map(float, ["-inf", "inf"])))
        )
        if bestbreaks is not None:
            breaks_set = breaks_set.difference(set(bestbreaks))
        ...
        for i in breaks_set:
            ...
            bestbreaks_i = (
                [float("-inf")]
                + sorted(bestbreaks + [i] if bestbreaks is not None else [i])
                + [float("inf")]
            )
            # best break datatable
            labels = [
                "[{},{})".format(bestbreaks_i[i], bestbreaks_i[i + 1])
                for i in range(len(bestbreaks_i) - 1)
            ]
            init_bin_all_breaks.loc[:, "bstbin" + str(i)] = pd.cut(
                init_bin_all_breaks["brkp"], bestbreaks_i, right=False, labels=labels
            )  # .astype(str)
        # best break dt
        total_iv_all_brks = (
            pd.melt(
                init_bin_all_breaks,
                id_vars=["variable", "good", "bad"],
                var_name="bstbin",
                value_vars=["bstbin" + str(i) for i in breaks_set],
            )
            .groupby(["variable", "bstbin", "value"], observed=False, group_keys=False)
            .agg({"good": "sum", "bad": "sum"})
            .reset_index()
            .assign(count=lambda x: x["good"] + x["bad"])
        )

        total_iv_all_brks["count_distr"] = (
            total_iv_all_brks.groupby(
                ["variable", "bstbin"], observed=False, group_keys=False
            )["count"]
            .apply(lambda x: x / dtm_rows)
            .reset_index(drop=True)
        )
        total_iv_all_brks["min_count_distr"] = total_iv_all_brks.groupby(
            ["variable", "bstbin"], observed=False, group_keys=False
        )["count_distr"].transform(lambda x: min(x))

        total_iv_all_brks = (
            total_iv_all_brks.assign(
                bstbin=lambda x: [float(re.sub("^bstbin", "", i)) for i in x["bstbin"]]
            )
            .groupby(
                ["variable", "bstbin", "min_count_distr"],
                observed=False,
                group_keys=False,
            )
            .apply(lambda x: iv_01(x["good"], x["bad"]))
            .reset_index(name="total_iv")
        )
        # return
    ...
    def binning_add_1bst(initial_binning, bestbreaks):
        if bestbreaks is None:
            ...
            if not is_numeric_dtype(dtm["value"]):
                bestbreaks = [
                    i for i in bestbreaks if int(i) != min(initial_binning.brkp)
                ]
            bestbreaks_inf = [float("-inf")] + sorted(bestbreaks) + [float("inf")]

        labels = [
            "[{},{})".format(bestbreaks_inf[i], bestbreaks_inf[i + 1])
            for i in range(len(bestbreaks_inf) - 1)
        ]
        binning_1bst_brk = initial_binning.assign(
            bstbin=lambda x: pd.cut(
                x["brkp"], bestbreaks_inf, right=False, labels=labels
            )
        )
        if is_numeric_dtype(dtm["value"]):
            binning_1bst_brk = (
                binning_1bst_brk.groupby(
                    ["variable", "bstbin"], observed=False, group_keys=False
                )
                .agg({"good": "sum", "bad": "sum"})
                .reset_index()
                .assign(bin=lambda x: x["bstbin"])[
            ...
            binning_1bst_brk = (
                ...
                .reset_index()[["bstbin", "variable", "bin", "good", "bad"]]
            )
        # format
        binning_1bst_brk["total_iv"] = iv_01(
            binning_1bst_brk.good, binning_1bst_brk.bad
        )
        binning_1bst_brk["bstbrkp"] = [
            float(re.match("^\[(.*),.+", i).group(1))
            for i in binning_1bst_brk["bstbin"]
        ]
        # return
    ...
    bstbrk_maxiv = total_iv_all_brks.loc[
        lambda x: x["min_count_distr"] >= count_distr_limit
    ]
    ...
    if bstbrk_maxiv is not None:
        ...
        bestbreaks = (
            bestbreaks + [bstbrk_maxiv] if bestbreaks is not None else [bstbrk_maxiv]
        )
    # binning add 1best break
...
def woebin2_tree(
    ...
    bin_list = woebin2_init_bin(
        dtm, init_count_distr=init_count_distr, breaks=breaks, spl_val=spl_val
    )
    ...
    while (IVchg >= stop_limit) and (step_num + 1 <= min([bin_num_limit, len_brks])):
        binning_tree = woebin2_tree_add_1brkp(
            dtm, initial_binning, count_distr_limit, bestbreaks
        )
        # best breaks
        bestbreaks = binning_tree.loc[
            lambda x: x["bstbrkp"] != float("-inf"), "bstbrkp"
        ].tolist()
        # information value
...
def woebin2_chimerge(
    ...
    def add_chisq(initial_binning):
        chisq_df = (
            pd.melt(
                initial_binning,
                id_vars=["brkp", "variable", "bin"],
                value_vars=["good", "bad"],
                var_name="goodbad",
                value_name="a",
            )
            .sort_values(by=["goodbad", "brkp"])
            .reset_index(drop=True)
        )
        ###
        chisq_df["a_lag"] = chisq_df.groupby("goodbad", group_keys=False)["a"].apply(
            lambda x: x.shift(1)
        )  # .reset_index(drop=True)
        chisq_df["a_rowsum"] = chisq_df.groupby("brkp", group_keys=False)[
            "a"
        ].transform(
            lambda x: sum(x)
        )  # .reset_index(drop=True)
        chisq_df["a_lag_rowsum"] = chisq_df.groupby("brkp", group_keys=False)[
            "a_lag"
        ].transform(
            lambda x: sum(x)
        )  # .reset_index(drop=True)
        ...
        chisq_df = (
            pd.merge(
                ...
                chisq_df.groupby("brkp", group_keys=False)
                .apply(lambda df: sum(df.a + df.a_lag))
                .reset_index(name="a_sum"),
            )
            .assign(
            ...
            .assign(
                ae=lambda df: (df.a - df.e) ** 2 / df.e
                + (df.a_lag - df.e_lag) ** 2 / df.e_lag
            )
            .groupby("brkp")
            .apply(lambda x: sum(x.ae))
            .reset_index(name="chisq")
        )
        # return
        return pd.merge(
            initial_binning.assign(count=lambda x: x["good"] + x["bad"]),
            chisq_df,
            how="left",
        )
    ...
    while (
        ...
        or bin_count_distr_min < count_distr_limit
        or bin_nrow > bin_num_limit
    ):
        # brkp needs to be removed
        if bin_chisq_min < chisq_limit:
            rm_brkp = (
                binning_chisq.assign(merge_tolead=False)
                .sort_values(by=["chisq", "count"])
                .iloc[0,]
            )
        ...
        # groupby brkp
        binning_chisq = (
            binning_chisq.groupby("brkp", group_keys=False)
            .agg(
                {
...
def woebin2(
    ...
    binning = (
        pd.concat(bin_list, keys=bin_list.keys())
        .reset_index()
...
def bins_to_breaks(bins, dt, to_string=False, save_string=None):
    ...
    bins_breakslist = bins[
        ~bins["breaks"].isin(["-inf", "inf", "missing"]) & ~bins["is_special_values"]
    ]
    bins_breakslist = pd.merge(
        bins_breakslist[["variable", "breaks"]], vars_class, how="left", on="variable"
    )
    bins_breakslist.loc[bins_breakslist["not_numeric"], "breaks"] = (
        "'" + bins_breakslist.loc[bins_breakslist["not_numeric"], "breaks"] + "'"
    )
    bins_breakslist = bins_breakslist.groupby("variable", group_keys=False)[
        "breaks"
    ].agg(lambda x: ",".join(x))
    ...
    if to_string:
        bins_breakslist = (
            "breaks_list={\n"
            + ", \n".join(
                "'" + bins_breakslist.index[i] + "': [" + bins_breakslist[i] + "]"
                for i in np.arange(len(bins_breakslist))
            )
            + "}"
        )
        if save_string is not None:
            brk_lst_name = "{}_{}.py".format(
                save_string, time.strftime("%Y%m%d_%H%M%S", time.localtime(time.time()))
            )
...
def woebin(
    ...
    **kwargs
    ...
    if init_count_distr is None:
        init_count_distr = 0.02
    ## count_distr_limit
    min_perc_coarse_bin = kwargs.get("min_perc_coarse_bin", None)
    if min_perc_coarse_bin is not None:
        count_distr_limit = min_perc_coarse_bin
    ## bin_num_limit
    ...
    if check_cate_num:
        check_cateCols_uniqueValues(dt, var_skip)
    ...
    if (
        init_count_distr < 0.01
        or init_count_distr > 0.2
        or not isinstance(init_count_distr, (float, int))
    ):
        warnings.warn(
    ...
    if (
        ...
        or count_distr_limit > 0.2
        or not isinstance(count_distr_limit, (float, int))
    ):
        warnings.warn(
    ...
    if not isinstance(bin_num_limit, (float, int)):
        warnings.warn(
            "Incorrect inputs; bin_num_limit should be numeric variable. Parameter was set to default (8)."
        )
        bin_num_limit = 8
    # method
    if method not in ["tree", "chimerge"]:
        warnings.warn(
            "Incorrect inputs; method should be tree or chimerge. Parameter was set to default (tree)."
        )
        method = "tree"
    ### ###
    ...
    if no_cores == 1:
        ...
        for i in np.arange(xs_len):
            ...
            bins[x_i] = woebin2(
                ...
                breaks=(
                    breaks_list[x_i]
                    if (breaks_list is not None) and (x_i in breaks_list.keys())
                    else None
                ),
                spl_val=(
                    special_values[x_i]
                    if (special_values is not None) and (x_i in special_values.keys())
                    else None
                ),
                init_count_distr=init_count_distr,
                count_distr_limit=count_distr_limit,
        ...
        args = zip(
            [
                pd.DataFrame({"y": dt[y], "variable": x_i, "value": dt[x_i]})
                for x_i in xs
            ],
            [
                (
                    breaks_list[i]
                    if (breaks_list is not None) and (i in list(breaks_list.keys()))
                    else None
                )
                for i in xs
            ],
            [
                (
                    ...
                    and (i in list(special_values.keys()))
                    else None
                )
                for i in xs
            ],
            [init_count_distr] * xs_len,
            [count_distr_limit] * xs_len,
            [stop_limit] * xs_len,
            [bin_num_limit] * xs_len,
            [method] * xs_len,
        )
        # bins in dictionary
    ...
    if runingtime >= 10 and print_info:
        ...
        print(
            "Binning on {} rows and {} columns in {}".format(
                dt.shape[0],
                dt.shape[1],
                time.strftime("%H:%M:%S", time.gmtime(runingtime)),
            )
        )
...
def woepoints_ply1(dtx, binx, x_i, woe_points):
    ...
    if is_numeric_dtype(dtx[x_i]):
        ...
        labels = [
            "[{},{})".format(breaks_binx_other[i], breaks_binx_other[i + 1])
            for i in range(len(breaks_binx_other) - 1)
        ]

        dtx = dtx.assign(
            xi_bin=lambda x: pd.cut(
                x[x_i], breaks_binx_other, right=False, labels=labels
            )
    ...
    try:
        dtx_suffix = (
            pd.merge(dtx, binx, how="left", on=x_i)
            .sort_values("rowid")
            #.drop_duplicates(subset="rowid")
            .set_index(dtx.index)[["_".join([x_i, woe_points])]]
        )
    except:
        print("For variable: {x_i} we have an error")
        dtx_suffix = (
            pd.merge(dtx, binx, how="left", on=x_i)
            .sort_values("rowid")
            .drop_duplicates(subset="rowid")
...
def woebin_ply(dt, bins, no_cores=None, print_step=0, replace_blank=True, **kwargs):
    ...
    if no_cores == 1:
        for i in np.arange(xs_len):
            ...
            dtx = dt[[x_i]]
            dat = pd.concat(
                [dat, woepoints_ply1(dtx, binx, x_i, woe_points="woe")], axis=1
            )
    else:
        pool = mp.Pool(processes=no_cores)
        # arguments
    ...
    if runingtime >= 10 and print_info:
        # print(time.strftime("%H:%M:%S", time.gmtime(runingtime)))
        print(
            "Woe transformating on {} rows and {} columns in {}".format(
                dt.shape[0], xs_len, time.strftime("%H:%M:%S", time.gmtime(runingtime))
            )
        )
    return dat
...
def plot_bin(binx, title, show_iv, output):
    """
    ...
    if (
        y_right_max > 1
        or y_right_max <= 0
        or y_right_max is np.nan
        or y_right_max is None
    ):
        y_right_max = 1
    ## y_left_max
    ...
    title_string = (
        binx.loc[0, "variable"]
        + "  (iv:"
        + str(round(binx.loc[0, "total_iv"], 4))
        + ")"
        if show_iv
        else binx.loc[0, "variable"]
    )
    ...
    ax1.set_xticks(ind, binx["bin"], rotation=10, ha="right", rotation_mode="anchor")
    plt.title(title_string, loc="left")
    plt.legend((p2[0], p1[0]), ("bad", "good"), loc="upper right")
    # plt.
    if output is not None:
        plt.savefig(output + title_string + ".png")
    plt.close()
    # show plot
    # plt.show()
    return fig
...
def woebin_plot(bins, x=None, title=None, show_iv=True, output=None):
    """
    ...
    #    plotlist[i] = plot_bin(binx, title, show_iv, output)
    with PdfPages(output + "bins.pdf") as pdf:
        for i in xs:
            binx = bins[bins["variable"] == i].reset_index(drop=True)
            fig = plot_bin(binx, title, show_iv, None)  # Create the figure
            plotlist[i] = fig
...
def woebin_adj_print_basic_info(i, xs, bins, dt, bins_breakslist):
    ...
    ## woebin plotting
    # plt.show(woebin_plot(binx)[x_i])   # modified
...
def woebin_adj_break_plot(dt, y, x_i, breaks, stop_limit, sv_i, method):
    ...
    breaks_bin = (
        ", ".join(breaks_bin)
        if is_numeric_dtype(dt[x_i])
        else ", ".join(["'" + i + "'" for i in breaks_bin])
    )
    print(">>> Current breaks:")
    print(breaks_bin, "\n")
    # print bin_adj
...
def woebin_adj(
    ...
    if not adj_all_var:
        bins2 = bins.loc[
            ~((bins["bin"] == "missing") & (bins["count_distr"] >= count_distr_limit))
        ].reset_index(drop=True)
        bins2["badprob2"] = (
            bins2.groupby("variable", observed=False, group_keys=False)
            .apply(lambda x: x["badprob"].shift(1))
            .reset_index(drop=True)
        )
        bins2 = bins2.dropna(subset=["badprob2"]).reset_index(drop=True)
        bins2 = bins2.assign(badprob_trend=lambda x: x.badprob >= x.badprob2)
        xs_adj = bins2.groupby("variable", observed=False, group_keys=False)[
            "badprob_trend"
        ].nunique()
    ...
    if xs_len == 0:
        warnings.warn(
            "The binning breaks of all variables are perfect according to default settings."
        )
        breaks_list = (
            "{"
            + ", ".join(
    ...
    while i <= xs_len:
        ...
        while adj_brk == 2:
            ...
            if breaks == "N":
                ...
                stop_limit = 0.1
            try:
                breaks = woebin_adj_break_plot(
                    dt, y, x_i, breaks, stop_limit, sv_i, method=method
                )
            except:
                pass
            # adj breaks again
