...
def var_filter(
    ...
    identical_perc = (
        dt[x]
        .apply(idt_rate)
        .reset_index(name="identical_rate")
        .rename(columns={"index": "variable"})
    )

    # dataframe iv na idt
    dt_var_selector = iv_list.merge(na_perc, on="variable").merge(
        identical_perc, on="variable"
    )
    # remove na_perc>95 | ele_perc>0.95 | iv<0.02
    # variable datatable selected
    dt_var_sel = dt_var_selector.query(
        "(info_value >= {}) & (missing_rate <= {}) & (identical_rate <= {})".format(
            iv_limit, missing_limit, identical_limit
        )
    )
    ...
    if runingtime >= 10:
        ...
        print(
            "Variable filtering on {} rows and {} columns in {} \n{} variables are removed".format(
                dt.shape[0],
                dt.shape[1],
                time.strftime("%H:%M:%S", time.gmtime(runingtime)),
                dt.shape[1] - len(x_selected + y),
            )
        )
    ...
    if return_rm_reason:
        dt_var_rm = dt_var_selector.query(
            ...
            info_value=lambda x: [
                "info_value<{}".format(iv_limit) if i else np.nan
                for i in (x.info_value < iv_limit)
            ],
            missing_rate=lambda x: [
                "missing_rate>{}".format(missing_limit) if i else np.nan
                for i in (x.missing_rate > missing_limit)
            ],
            identical_rate=lambda x: [
                "identical_rate>{}".format(identical_limit) if i else np.nan
                for i in (x.identical_rate > identical_limit)
            ],
        )
        dt_rm_reason = (
            pd.melt(dt_var_rm, id_vars=["variable"], var_name="iv_mr_ir")
            .dropna()
            .groupby("variable")
            .apply(lambda x: ", ".join(x.value))
            .reset_index(name="rm_reason")
        )
        ...
        if var_rm is not None:
            dt_rm_reason = pd.concat(
                [
                    dt_rm_reason,
                    pd.DataFrame(
                        {"variable": var_rm, "rm_reason": "force remove"},
                        columns=["variable", "rm_reason"],
                    ),
                ]
            )
        if var_kp is not None:
        ...
        dt_rm_reason = pd.merge(
            dt_rm_reason, dt_var_selector, how="outer", on="variable"
        )
