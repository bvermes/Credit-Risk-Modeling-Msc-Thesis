...
def iv(dt, y, x=None, positive="bad|1", order=True):
    ...
    # info_value
    ivlist = pd.DataFrame(
        {"variable": xs, "info_value": [iv_xy(dt[i], dt[y[0]]) for i in xs]},
...
def iv_xy(x, y):
    ...
    # iv calculation
    iv_total = (
        pd.DataFrame({"x": x.astype("str"), "y": y})
        .fillna("missing")
        .groupby("x")
        .apply(goodbad)
        .replace(0, 0.9)
        ...
        .assign(
            iv=lambda x: (x.DistrBad - x.DistrGood) * np.log(x.DistrBad / x.DistrGood)
        )
        .iv.sum()
    )
    # return iv
...
def iv_01(good, bad):
    # iv calculation
    iv_total = (
        pd.DataFrame({"good": good, "bad": bad})
        .replace(0, 0.9)
        .assign(
            DistrBad=lambda x: x.bad / sum(x.bad),
            DistrGood=lambda x: x.good / sum(x.good),
