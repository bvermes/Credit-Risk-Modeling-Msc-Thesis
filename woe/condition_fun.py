...
def lr(dt, y, x):
    ...
    # logistic regression
    lrfit = sm.GLM(
        dty.astype(float), dtx.astype(float), family=sm.families.Binomial()
    ).fit()
    return lrfit
...
def check_datetime_cols(dat):
    datetime_cols = (
        dat.apply(pd.to_numeric, errors="ignore")
        .select_dtypes(object)
        .apply(pd.to_datetime, errors="ignore", format="%Y-%m-%d %H:%M:%S")
...
def rep_blank_na(dat):  # cant replace blank string in categorical value with nan
    ...
    if dat.index.duplicated().any():
        ...
        warnings.warn(
            "There are duplicated index in dataset. The index has been reseted."
        )
    ...
    blank_cols = [
        ...
        .astype(str)
        .str.findall(r"^\s*$")
        .apply(lambda x: 0 if len(x) == 0 else 1)
        .sum()
        > 0
    ]
    if len(blank_cols) > 0:
        warnings.warn(
            "There are blank strings in {} columns, which are replaced with NaN. \n (ColumnNames: {})".format(
    ...
    if len(cols_num) > 0:
        cols_inf = [
            col
            for col in cols_num
            if any(dat[col] == np.inf) | any(dat[col] == -np.inf)
        ]
        if len(cols_inf) > 0:
            warnings.warn(
...
def check_y(dat, y, positive):
    ...
    if not isinstance(dat, pd.DataFrame):
        ...
        raise Exception(
            "Incorrect inputs; dat should be a DataFrame with at least two columns."
        )
    ...
    if dat[y].isnull().any():
        warnings.warn(
            "There are NaNs in '{}' column. The rows with NaN in '{}' were removed from dat.".format(
                y, y
            )
        )
    ...
    if len(unique_y) == 2:
        ...
        if True in [bool(re.search(positive, str(v))) for v in unique_y]:
            ...
            if (y1 != y2).any():
                ...
                warnings.warn(
                    'The positive value in "{}" was replaced by 1 and negative value by 0.'.format(
                        y
                    )
                )
        else:
            raise Exception(
                'Incorrect inputs; the positive value in "{}" is not specified'.format(
                    y
                )
            )
    else:
        raise Exception(
            "Incorrect inputs; the length of unique values in y column '{}' != 2.".format(
                y
            )
        )
    # to int
...
def check_print_step(print_step):
    if not isinstance(print_step, (int, float)) or print_step < 0:
        warnings.warn(
            "Incorrect inputs; print_step should be a non-negative integer. It was set to 1."
        )
        print_step = 1
    return print_step
